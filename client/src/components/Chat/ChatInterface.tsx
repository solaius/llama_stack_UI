    ...
    try {
      ...
      if (isStreaming) {
        ...
        let assistantMessage: Message = { 
          role: 'assistant', 
          content: '',
          stop_reason: 'end_of_turn' // Default stop reason
        };
          ...
          eventSource.onmessage = (event) => {
            ...
            try {
              ...
              if (data.event && data.event.event_type === 'progress') {
                ...
                // Error message
                console.error('Error in stream:', data.error, data.message);
                setMessages(prev => [
                  ...prev.slice(0, -1),
                  {
                    role: 'assistant',
                    content: `Sorry, there was an error: ${data.message || data.error}`,
                  },
                ]);
                setIsLoading(false);
                eventSource.close();
